...
export const useCartStore = defineStore('cart', {
  state: () => {
    ...
    const appliedCoupon = currentUser?.appliedCoupon || { code: '', discount: 0 }

    return {
      items: currentUser?.cartItems as CartItem[] || [],
  ...
  getters: {
    ...
    discountAmount: (state) => (discount: number) =>
      state.items.reduce((total, item) => total + item.currentPrice * item.quantity, 0) * discount,
  ...
  actions: {
    ...
    applyVoucher(code: string, discount: number) {
      ...
      if (userIndex !== -1) {
        users[userIndex].appliedCoupon = { code, discount }
        localStorage.setItem('users', JSON.stringify(users))
      }
    },
    ...
    removeVoucher() {
      ...
      if (userIndex !== -1 && users[userIndex].appliedCoupon) {
        delete users[userIndex].appliedCoupon
        localStorage.setItem('users', JSON.stringify(users))
      }
    }
  }
